...
- **Container runtime** — `containers.rs` is the CRUD/lifecycle/inspection orchestrator; it delegates to a set of focused sibling modules rather than doing everything itself: `container_schema.rs` (the `Environment` struct and its defaults), `container_validation.rs` (all `Environment` field validation), `container_compose.rs` (docker-compose/Dockerfile template generation), `container_bootstrap.rs` (per-database readiness polling and create-if-missing), `container_gateway.rs` (the shared nginx gateway: routing, TLS wiring, recreate-on-restart retries), `container_logs.rs` (log streaming), `container_lifecycle.rs`, `container_inspection.rs`, `container_routes.rs`, and `container_runtime.rs` (auto-detects `docker` or `podman` on `PATH`, configurable, and the matching Compose provider).
