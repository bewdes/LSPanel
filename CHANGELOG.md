...
- Auto-heal ran its first check the instant LS Panel launched, with no warm-up delay. On a machine with more than one environment that was running last session, that instant check would catch every one of them still mid-startup, misread that as a crash, and both fire an alarming "service issue" notification and restart it — which also looked like starting one project was somehow also starting the others. Auto-heal now waits 90 seconds before its first check.
